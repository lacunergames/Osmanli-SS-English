...
async function check() {
  const { data, error } = await supabase.from('vocabulary_master').select('*').limit(5);
  console.log("vocabulary_master:", data, error);
  
  const { data: data2, error: error2 } = await supabase.from('vocabulary').select('*').limit(5);
  console.log("vocabulary:", data2, error2);
}
